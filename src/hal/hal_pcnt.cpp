...
static void gate_timer_callback(void *arg) {
    ...
    int64_t total_pulses = ((int64_t)overflows * PCNT_H_LIM_VAL) + (count > 0 ? count : 0);
    if (total_pulses < 0) total_pulses = 0;

    double elapsed_sec = (double)elapsed_us / 1000000.0;
    double freq_hz = (double)total_pulses / elapsed_sec;
    double period_us = (freq_hz > 0.001) ? (1000000.0 / freq_hz) : 0.0;

    if (xSemaphoreTake(s_meas_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        s_latest_measurement.frequency_hz = freq_hz;
        s_latest_measurement.period_us = period_us;
        s_latest_measurement.duty_cycle_pct = (freq_hz > 0.01) ? 50.0 : 0.0;
        s_latest_measurement.pulse_width_us = period_us * 0.5;
...
esp_err_t hal_pcnt_init(const pcnt_config_params_t *config) {
    ...
    if (s_config.input_gpio == 0) s_config.input_gpio = PIN_FREQ_INPUT;
    ...
    s_is_inited = true;
    ESP_LOGI(TAG, "Hardware PCNT initialized on GPIO %d (Clean hardware counter)", s_config.input_gpio);
    return ESP_OK;
}
...
esp_err_t hal_pcnt_start(void) {
    ...
    if (err == ESP_OK) {
        s_is_running = true;
        ESP_LOGI(TAG, "PCNT counter started (Gate: %lu ms)", (unsigned long)s_config.gate_time_ms);
    }
    return err;
}
